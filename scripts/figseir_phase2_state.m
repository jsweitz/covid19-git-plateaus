...
% figure is drawn
tmpfilename = 'figseir_phase2_state';
...
for i=1:length(pars.Dtot_crit_range),
  ...
  [t,y]=ode45(@seir_plateau_state,[0:0.1:400],y0,opts,pars);
  S=y(:,1);
  E=y(:,2);
  I=y(:,3);
  R=y(:,4);
...
tmpt=text(0.52,10^3.2,'(D)','Fontsize',20,'fontweight','bold');

% legend
% tmplh = legend('stuff',...);
tmplh = legend('$ND_{c}=1,000$ deaths' ,'$ND_{c}=2,000$ deaths','$ND_{c}=5,000$ deaths');
...
% Create arrow
annotation(gcf,'arrow',[0.488299531981279 0.435257410296412],...
    [0.617903930131004 0.637554585152837],'LineWidth',2);


% Create arrow
annotation(gcf,'arrow',[0.466458658346334 0.535101404056162],...
    [0.783842794759825 0.779475982532751],'LineWidth',2);
