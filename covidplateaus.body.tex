...
%(JD: Predefined not clear, maybe wrong)
...
%In many cases, epidemic models
%might further categorize individuals by age and disease status, or age, occupation,
%and disease status, and so on.  
%As a result, increases in cases
...
(JD: I don't think we need anything from this \P. Much of it is also awkward and hard to understand. Stochastic amplification tends to only be interesting for damped or really small oscillations.) In essence, delays between infection and fatalities can lead to larger oscillations,
...
(JD: Don't even imply avoidance leads to more death.)
In summary, we have shown how awareness-driven avoidance of transmission
in response to disease-induced death can lead
to highly asymmetric epidemic curves, where the disease declines 
even as the majority of the population remains susceptible, but may do so very slowly. 
...
In particular, passing a `peak' need not imply
