...
\jd{This will actually need a fair amount of work to explain how relevant simple, symmetric models still are (and aren't) [sad face]}
\jd{Weitz, do you have a special script to insert illogical line breaks in the .tex?}
\jd{Maybe less emphasis on Gaussian symmetry and more emphasis on the broader world of symmetric-like models, including SEIR.}
...
of SARS-CoV-1 may prepare individuals to more
...
%robust to variation in assumptions on the 
%quantitative and qualitative nature of awareness-driven distancing.
To test this mechanistic link, we jointly analzed the dynamics of fatality
...
implies a collective response akin to opening up too early.
For example, in the case of North Carolina, mobility increased even as fatality
rates were increasing. Whereas, the state of New York exhibited reversible
dynamics in the phase plane, implying that decreases in mobility were
tightly linked to fatality rates.
...
awareness that would lead to sustained declines in fatalities).
...
We analyze mobility data using principal component analysis (PCA) in order to obtain an aggregated mobility proxy. 
We exclude mobility measures for parks from the analysis due to their anomalous, noisy patterns (Supplementary Figure~\ref{fig.supp_mobility}).
Before performing PCA, we first calculate the 7-day rolling average for each mobility measure in order to remove the effects of weekdays.
Mobility measures are further centered to have zero mean and scaled to unit variance. We then perform a single PCA using a combined mobility data set across all 17 states (instead of performing independent PCAs for each state). 
Estimated principal components are further smoothed using locally locally estimated scatterplot smoothing to draw a continuous-time phase plain.
We focus on the first principal component, which explains 93\% of the total variance, in our phase plain analysis. 
The first principal component is negatively correlated with all mobility measures, except for mobility in the residential area. 
