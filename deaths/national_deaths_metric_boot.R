...
g1 <- ggplot(us_death_fit0a) +
  geom_line(aes(day2, pred, group=region, col=region, lty=region)) +
  geom_point(data=us_death_fit0a_end, aes(day2, pred, group=region, fill=region), shape=21, col=1) +
  scale_x_continuous("Time since $t_1$ (days)", expand=c(0, 0), limits=c(0, 78)) +
  scale_y_log10("Smoothed daily number of deaths") +
  scale_color_manual(values=rep(c(1,2), 8)) +
  scale_fill_manual(values=rep(c(1,2), 8)) +
  scale_linetype_manual(values=c(1:9, 1:9))

g1a <- direct.label(g1, list("last.bumpup", hjust=-0.2))
