...
set(gca,'fontsize',20);
hold on
tmph=plot(t,pars.N*pars.Dcrit*(pars.R0^(1/pars.awareness)-1)*ones(size(t))/pars.frac_D,'k--');
set(tmph,'linewidth',3,'color',[0.2 0.2 0.8]);
tmpt=text(200,1.075*pars.N*pars.Dcrit*(pars.R0^(1/pars.awareness)-1)/pars.frac_D,'Infection rate plateau, $\dot{I}^{(q)}$');
set(tmpt,'interpreter','latex','fontsize',18);
title({'(B) SEIR Model with Death-Awareness';'$N=10^7$, $D_c=100$ deaths/day, $a=2$'}','fontsize',20,'interpreter','latex');
set(gca,'xticklabel',[]);
ylim([10^0 10^4]);
...
set(gca,'fontsize',20);
hold on
tmph=plot(t,pars.N*pars.Dcrit*(pars.R0^(1/pars.awareness)-1)*ones(size(t)),'k--');
set(tmph,'linewidth',3);
tmpt=text(200,1.075*pars.N*pars.Dcrit*(pars.R0^(1/pars.awareness)-1),'Fatality rate plateau, $\dot{D}^{(q)}$');
set(tmpt,'interpreter','latex','fontsize',18);
