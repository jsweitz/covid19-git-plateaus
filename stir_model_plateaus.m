function dydt = stir_model_plateaus(t,y,pars,agepars)
% function dydt = stir_model_plateaus(t,y,pars,agepars)
...
% Dynamics - Open
% Susceptibles
dydt(agepars.S_ids)=-pars.beta_a*y(agepars.S_ids)*Ia/(1+(Ddot/pars.Dc)^pars.awareness)-pars.beta_s*y(agepars.S_ids)*Is/(1+(Ddot/pars.Dc)^pars.awareness)+agepars.ageleave'.*y(agepars.Slock_ids);
dydt(agepars.E_ids)=pars.beta_a*y(agepars.S_ids)*Ia/(1+(Ddot/pars.Dc)^pars.awareness)+pars.beta_s*y(agepars.S_ids)*Is/(1+(Ddot/pars.Dc)^pars.awareness)+agepars.ageleave'.*y(agepars.Slock_ids) -pars.gamma_e*y(agepars.E_ids);
